(function(){
  ...
  var whens=[];
  function computeWhens(){ whens=[['all','Any date']]; var seen={};
    suites.slice().sort(function(a,b){ return (isNow(a.available)?'0':a.available) < (isNow(b.available)?'0':b.available)?-1:1; }).forEach(function(s){ var k=monthKey(s.available); if(!seen[k]){ seen[k]=1; whens.push([k, k==='now'?'Now':months[+k.slice(5,7)-1]]); } });
  }
  computeWhens();
  var fType='all', fWhen='all';
  ...
  function buildWhenChips(){ computeWhens(); fWhen='all'; chips(document.getElementById('whenChips'),whens,function(v){fWhen=v;}); }
  buildWhenChips();
  var money=function(n){ return '$'+n.toLocaleString('en-CA'); };
  var list=document.getElementById('suiteList');
  function render(){
    var rows=suites.filter(function(s){ return (fType==='all'||s.type===fType) && (fWhen==='all'||monthKey(s.available)===fWhen); })
    ...
    list.innerHTML = rows.length ? rows.map(function(s){
      var hasPhoto=s.photos.length>0, img=hasPhoto?s.photos[0]:s.planThumb;
      return '<article class="suite">'+
        '<button class="suite-media'+(hasPhoto?'':' plan')+'" data-unit="'+s.unit+'" aria-label="View floor plan and photos for suite '+s.unit+'">'+
          '<img src="'+img+'" alt="'+(hasPhoto?'Photo of suite '+s.unit:'Floor plan of suite '+s.unit)+'" loading="lazy">'+
          '<span class="when'+(isNow(s.available)?' now':'')+'">'+whenLabel(s.available)+'</span>'+
          '<span class="media-tag">'+(hasPhoto?(s.photos.length+' photos + floor plan'):'View floor plan')+'</span>'+
        '</button>'+
        ...
        '</div></article>';
    }).join('') : '<div class="empty">No suites match right now. <a href="#contact">Send us a message</a> and we\'ll let you know when one opens up.</div>';
  }
  render();
  function updateCounts(){
    var nowCount=suites.filter(function(s){return isNow(s.available);}).length;
    document.getElementById('availCount').textContent = nowCount>0 ? (nowCount+' suites available now') : (suites.length+' suites coming available');
    if(suites.length) document.getElementById('fromPrice').textContent='From '+money(Math.min.apply(null,suites.map(function(s){return s.price;})));
  }
  updateCounts();
  ...
  var galleries={
    ...
    building:{title:'The building',sub:'325 University Ave W, Cobourg',items:[['img/amenity-lounge.jpg','Resident lounge'],['img/amenity-kitchen.jpg','Lounge kitchen'],['img/ext-ev.jpg','EV charging'],['img/ext-sign.jpg','Building sign'],['img/amenity-wide.jpg','Lounge'],['img/aerial-corner.jpg','Building exterior'],['img/aerial-garden.jpg','Rooftop terraces'],['img/ext-trees.jpg','Street view']]}
  };
